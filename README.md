...
这些数字表示从 `1` 开始计算的屏幕目标列，并不是要插入的固定空格数。两个相邻目标列之差表示前一个字段预留的显示宽度：

| 配置项 | 默认列 | 含义 |
| --- | ---: | --- |
| `direction` | 5 | `input` 或 `output` 的起始列；前面默认保留 4 列缩进 |
| `type` | 13 | `wire` 的起始列；方向字段默认占用第 5～12 列 |
| `width` | 21 | `[` 和位宽值的起始列；类型字段默认占用第 13～20 列 |
| `width_suffix` | 29 | `-1: 0]` 的起始列，使不同长度的位宽值保持右侧对齐 |
| `name` | 45 | 信号名的起始列 |
| `comma` | 90 | 行尾逗号所在列；信号名字段默认延伸到第 89 列 |

如果位宽表达式或信号名超过预留宽度，生成器不会覆盖或截断内容，而会保留至少一个空格并将后面的字段顺延。修改配置后重新执行 `:source $MYVIMRC` 即可生效。

输入位宽为 `1` 时按标量端口处理，不生成 `[1-1: 0]` 范围；信号名和行尾逗号仍保持在配置的目标列。
...
`,in`、`,tb`、`,ii` 和 `,oo` 的输入格式、生成内容和后续优化计划见 [自定义 Verilog 生成脚本](docs/custom-generators.md)。
